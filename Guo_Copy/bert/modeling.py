...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    'bertu': "/netscratch/abusuttil/Guo_copy/bertu.tar.gz",
    'mbertu': "/netscratch/abusuttil/Guo_copy/mbertu.tar.gz",
}
...
class BertConfig(object):
    ...
    def __init__(self,
                ...
                type_vocab_size=2,
                initializer_range=0.01,  # Adjust based on stability
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2
           ...
           # self.hidden_act_2 = hidden_act_2
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        if input_ids.max() >= num_embeddings:
         #   print("Found out-of-range index in input_ids")
            print(input_ids.max())
        ...
        max_position_id = self.position_embeddings.weight.size(0) - 1
        position_ids = torch.clamp(position_ids, max=max_position_id)
       # print(f"Clamped position ids to max {max_position_id}: {position_ids}")
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if isinstance(config.hidden_act, str) or (sys.version_info[0] == 2 and isinstance(config.hidden_act, unicode)):
            ...
            #self.intermediate_act_fn_2 = ACT2FN[config.hidden_act_2]
        else:
            self.intermediate_act_fn = config.hidden_act
            #self.intermediate_act_fn_2 = config.hidden_act_2
...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        if isinstance(config.hidden_act, str) or (sys.version_info[0] == 2 and isinstance(config.hidden_act, unicode)):
            ...
            #self.transform_act_fn_2 = ACT2FN[config.hidden_act_2]
        else:
            self.transform_act_fn = config.hidden_act
            #self.transform_act_fn_2 = config.hidden_act_2
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
